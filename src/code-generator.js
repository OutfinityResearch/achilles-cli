...
class CodeGenerator {
    constructor() {
        ...
        this.runOnce = ['1', 'true', 'yes'].includes(String(process.env.CODEGEN_RUN_ONCE || '').toLowerCase());
        this.watcher = null;
    ...
    watchSpecsDirectory() {
        ...
        this.watcher = fs.watch(specsDir, { recursive: true }, async (eventType, filename) => {
            if (eventType === 'change' && filename.endsWith('.spec')) {
    ...
    getCodePath(specPath) {
        ...
        const codePath = path.join(process.cwd(), relativePath.replace(/\.spec$/i, ''));
        return codePath;
    }
        ...
        try {
            ...
            if (Array.isArray(context.designSpecs) && context.designSpecs.length > 0) {
                context.designSpecs.forEach((design) => {
                    llmContext.push({
                        role: 'system',
                        content: `Design spec (${design.path}):\n${design.content}`
                    });
                });
            }
